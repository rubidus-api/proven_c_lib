...
int main(void) {
    PROVEN_TEST_SUITE(
        "test_float_f32_boundary_neighbors",
        "Verify the float32 upgrade and shortest corpus pin the ULP-adjacent neighbors around FLT_MIN and FLT_TRUE_MIN.",
        "Inspect tests/test_float_upgrade_corpus.c or tests/test_float_shortest_roundtrip.c if a float32 boundary-neighbor case disappears."
    );

    char *upgrade = read_file_text("tests/test_float_upgrade_corpus.c");
