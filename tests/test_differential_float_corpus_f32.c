...
static void require_text_present(const char *text, const char *needle, const char *label) {
    PROVEN_TEST_ASSERT(strstr(text, needle) != NULL, label, "Inspect tests/test_differential_float_corpus_f64.c if the float32 shortest corpus coverage disappears.");
}

int main(void) {
    PROVEN_TEST_SUITE(
        "test_differential_float_corpus_f32",
        ...
        "Inspect tests/test_differential_float_corpus_f64.c if the float32 coverage section disappears or drifts from the documented literals."
    );

    char *src = read_file_text("tests/test_differential_float_corpus_f64.c");
    ...
    PROVEN_TEST_SECTION(
        ...
        "Inspect tests/test_differential_float_corpus_f64.c if any documented float32 literal is missing from the corpus section."
    );
