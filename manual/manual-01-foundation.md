...
modify it, hand one row to code that may modify only that row, delete a row by
moving the rows after it down over it, and decide whether a pointer that arrived
from somewhere else even points into the table.
...
`proven_mem_copy` documents non-overlapping regions. It will not report this; it
...
If `ptr` came from a different allocation, this comparison is undefined
behaviour, not merely a comparison that returns the wrong answer.
...
```text
#define PROVEN_VERSION_STRING "proven_c_lib-v26.09.02b"
