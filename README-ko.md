...
- 버전: proven_c_lib-v26.07.20d · 표준: C23 · 라이선스: MIT
...
```c
int a[4], b[4];
int *p = a + 4;            /* a의 끝 바로 다음 */
int *q = b;                /* b의 시작 */
/* 실행 시점에 p와 q가 같게 비교될 수도 있다. 그래도 같은 포인터가 아니다:
   *p는 미정의 동작이고, *q는 멀쩡하다. 비트는 같아도 provenance가 다르다. */
```

이것은 표준 문헌 애호가의 호기심거리가 아닙니다. 컴파일러가 "`p`를 통한 쓰기는 `b`를 건드릴 수 없다"고
가정해도 되는 근거이며, 최적화의 상당 부분이 여기서 나옵니다. 그리고 포인터를 정수로 세탁해서 쓰는
프로그램이, 마치 컴파일러가 고장 난 것처럼 보이는 방식으로 잘못 컴파일되는 이유이기도 합니다.

이 모델은 진행 중인 실제 표준화 작업입니다. ISO WG14의 provenance 기술 명세
