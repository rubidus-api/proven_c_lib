...
int main(int argc, char **argv)
{
    ...
    const Proven_Test_Case all_tests[] = {
        ...
        { "tests/test_float_shortest_split", "float shortest helper split", "Verify the shortest float-format backend keeps thin width-specific shims over one shared parser-driven helper.", "Inspect src/proven/float_format.c if the shortest backend loses the shared helper or collapses the width-specific shims too early." },
        ...
        { "tests/test_float_shortest_binary_search", "float shortest parser-driven backend", "Verify the shortest float-format backend uses a parser-driven round-trip helper while preserving width-specific precision boundaries.", "Inspect src/proven/float_format.c if the shortest backend reverts to the old precision helper or collapses the width-specific shims." },
        ...
        { "tests/test_float_shortest_common_helper", "float shortest common helper", "Verify the shortest float-format backend routes per-width wrappers through one shared shortest helper with binary64 and binary32 precision limits intact.", "Inspect src/proven/float_format.c if the wrappers stop calling the shared shortest helper or lose their width-specific limits." },
