...
int main(int argc, char **argv)
{
    ...
    const Proven_Test_Case all_tests[] = {
        ...
        { "manual/examples/ex_00_hello", "manual example: the first program", "Compile and run the hello-world program printed in manual chapter 0 - the one a new reader copies first.", "Chapter 0 prints this program verbatim and calls it the shortest complete program. If it fails, the first thing a reader tries does not work - fix the example, then re-copy it into the chapter." },
        ...
        { "manual/examples/ex_04_ring", "manual example: the ring buffer", "Compile and run the ring-buffer example printed in manual chapter 4 - push until full, the refusal, pop in order, and the empty case.", "The chapter says a full ring refuses rather than overwrites and an empty one refuses rather than invents. If this fails, one of those two promises is no longer true." },
        { "manual/examples/ex_04_list", "manual example: the intrusive list", "Compile and run the intrusive-list example printed in manual chapter 4 - links inside the caller's struct, walking with PROVEN_LIST_ENTRY, and removing safely while iterating.", "The chapter claims inserting and removing allocate nothing and cannot fail, and that the SAFE walk is required when removing. If this fails, the chapter is teaching a list that does not behave that way." },
        ...
        { "manual/examples/ex_05_time", "manual example: time as a duration and as a date", "Compile and run the time example printed in manual chapter 5 - elapsed nanoseconds across a sleep, the broken-down calendar fields, and formatting a timestamp.", "The chapter distinguishes measuring a duration from reading a calendar, and states the field ranges. If this fails, either the clock or the breakdown no longer matches what chapter 5 promises." },
