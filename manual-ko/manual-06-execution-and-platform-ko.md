...
계약, 프리스탠딩(freestanding) 모드, 플랫폼 빌드 관련 사항을 다룬다. 이것은 이 매뉴얼에서 가장 어려운
내용이며, 의도적으로 맨 마지막에 놓았다 — 1–5부의 어떤 것도 여기에 의존하지 않는다.

## 목차

1. [스택리스 코루틴](#1-스택리스-코루틴)
2. [Job system](#2-job-시스템)
...
## 2. Job 시스템
작업(job) 시스템은 워커 스레드와 경계가 있는 MPMC 방식 큐를 소유한다. 생산자는 `routine(arg)` 작업 항목을 제출한다. 워커는 시스템이 닫히고 비워질 때까지 큐에 들어온 작업을 실행한다.
...
- `proven_job_system_close()`는 제출자와 경쟁할 수 있다. close 전에 진입을 허가받은
  제출은 close가 반환되기 전에 작업을 commit하거나 `false`를 반환하며, 그 이후의
  제출은 거부된다.
- `proven_job_system_destroy()`는 여전히 `proven_job_submit()`을 호출하는 생산자 스레드와 경쟁(race)해서는 안 된다.
- close를 정지 신호로 사용하려면 close를 호출하고, 모든 생산자를 join한 뒤에만
  destroy해야 한다.
...
- 생산자(`proven_job_submit`)는 다음 enqueue 위치를 원자적으로 확보한다. 확보가
  ...
  용량이 생기기를 기다리거나 대기 중인 작업을 덮어쓰는 대신 `false`를 반환한다.
  워커 wake를 post하는 과정에서는 플랫폼 동기화에 잠시 진입할 수 있다.
...
유휴 워커는 계속 폴링하고 yield하는 대신 플랫폼 카운팅 세마포어에서 블록한다.
성공한 submit은 슬롯을 먼저 공개한 뒤 permit 하나를 post한다. 대기 중인 워커가
없어도 permit은 남으므로 빈 큐를 확인한 시점과 대기에 들어가는 시점 사이에서 wake가
유실되지 않는다. `close`는 먼저 새 진입을 막고 이미 허가된 submit이 끝나기를 기다린
다음, 워커마다 최종 permit 하나를 post한다. 외부
`proven_job_execute_one`이 워커보다 먼저 작업을 가져가면 남은 permit은 단지
stale 상태가 된다. 워커는 빈 큐를 다시 확인한 뒤 다시 대기하거나, close 뒤라면
종료한다.
...
그 작업이 사용하는 할당자(allocator)의 스레드 안전성, 할당을 공유할 때의 포인터 프로버넌스
...
- `close`는 플래그를 뒤집어 이후의 모든 `submit`이 `false`를 반환하게 한다. 이미
  허가된 submit은 close가 반환되기 전에 정리되며, 수락된 모든 작업은 여전히 실행된다.
...
2. **Arena와 풀(pool)은 공유해서는 안 된다.** 동시 `proven_arena_alloc`은 `offset`을
...
- **주소 재사용 / ABA.** `free` 후 `alloc`(특히 pool의 프리리스트 재활용)은 *같은
  ...
  여전히 UB다.
- **`uintptr_t` 왕복 + 찢어진 읽기.** 아레나(arena)는 정렬 계산을 위해 `backing.ptr`을
...
C에서 접두사는 장식이 아니다. 이 언어에는 함수를 위한 전역 이름 공간이 하나뿐이므로, 슬라이스(slice)를 뜻하는 `slice`나
...
- `heap` — 힙(heap): 컴파일되지만, `proven_heap_allocator()`는 유효하지 않은 0 allocator를 반환한다.
...
설정된 MinGW 경로는 smoke 실행 파일을 추가로 링크한다. 약해 보이지만 그렇지 않다:
**대부분의 이식성 실패는 컴파일 타임 실패다.** 가정한 폭이 아닌 타입,
...
- MinGW를 통한 Windows x86_64 및 i686 WinAPI 컴파일·링크 smoke 검사.
...
- 크로스 빌드는 설정된 MinGW 컴파일·링크 smoke를 제외하면 컴파일 전용이며, 어느 경로도
  런타임 검증은 아니다.
...
```text
proven_job_system_close(sys); /* submit과 경쟁할 수 있으며 submit을 false로 만든다 */
join_producer_threads();      /* 이 지점 뒤에는 어떤 생산자도 sys에 접근하지 않는다 */
proven_job_system_destroy(sys);
```
