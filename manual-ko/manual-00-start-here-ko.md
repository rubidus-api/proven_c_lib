...
6. [의도와 설계 철학](#6-의도와-설계-철학)
7. [빌드와 include 모델](#7-빌드와-include-모델)
8. [전역 계약(Global contracts)](#8-전역-계약global-contracts)
9. [소유권과 파괴 매트릭스](#9-소유권과-파괴-매트릭스)
10. [연산 동작 클래스](#10-연산-동작-클래스)
11. [매뉴얼 챕터](#11-매뉴얼-챕터)
12. [플랫폼 지원과 검증](#12-플랫폼-지원과-검증)
13. [부록 B: 용어집](#13-부록-b-용어집)
14. [부록 C: 공개 헤더 맵](#14-부록-c-공개-헤더-맵)
15. [부록 D: libc 대응표](#15-부록-d-libc-대응표)
16. [다음에 읽을 것](#16-다음에-읽을-것)
...
이 하나하나는 처음 등장하는 자리에서 설명하며, 전부 §13의 용어집에도 있습니다.
...
식입니다 — 아래 §14가 모든 헤더를 그것을 문서화한 챕터로 연결해 줍니다.
...
모든 곳에서는 전제로 깔립니다. 형식적인 버전은 아래 §8에 있고, 여기 있는 것은 일상
언어판입니다.
...
아래 §9.2에 이런 타입 열여섯 개가 모두 나열되어 있습니다. 규칙은 간단합니다. 살아 있을
...
여기까지가 일상 언어로 쓴 입문입니다. 이 장의 나머지는 다른 장들이 기대는 레퍼런스입니다:
라이브러리의 의도와 빌드 모델을 형식적으로 적은 것, 전역 계약, 소유권 매트릭스, 연산 동작
클래스, 읽는 순서, 플랫폼 지원, 그리고 부록들. 예전에는 매뉴얼의 첫 페이지인 별도 문서였는데,
첫 페이지를 차례만으로 두기 위해 이곳으로 옮겼습니다.
...
이들은 [§4.2](#92-caller-owned-state--destroy-없음-복사-금지)에 정리되어 있습니다.
...
§5](#5-모든-페이지에서-만나게-될-다섯-가지-계약)가 그 이유를 설명합니다.
...
숙련된 C 프로그래머이고 시간이 없다면, 위의 §15를 읽고, 그다음
