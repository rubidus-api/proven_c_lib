...
- 버전: proven_c_lib-v26.07.20e · 표준: C23 · 라이선스: MIT
...
C의 메모리 모델은 거의 정확히 같은 뜻으로 이 단어를 씁니다. 추상 기계에서 **포인터는 주소만이 아닙니다**
— 그것이 유래한 저장소의 정체성까지 함께 지닙니다. 두 포인터가 같은 수치를 담고도 서로 대체할 수 없을
수 있습니다. 서로 다른 객체에서 왔기 때문입니다.

```c
int a[4], b[4];
int *p = a + 4;   /* 합법: C는 배열 마지막 원소 바로 다음을 가리키는 포인터 형성을 허용한다 */
int *q = b;       /* 다른 배열의 시작 */
/* p와 q는 같은 주소를 담을 수 있고, 둘을 비교하는 것조차 허용된다. 허용되지 않는 것은
   p로 b에 닿는 것이다. p는 b가 아니라 a의 정체성을 지니며, 컴파일러가 그것에 의존한다 —
   p를 통한 쓰기가 b를 건드릴 수 없다고 가정한다. 주소가 우연히 겹친다고 해서 두 포인터가
   대체 가능해지는 것은 아니다. */
```

여기서 **무엇이 문제가 아닌지**에 주목하세요. `a + 4`를 형성하는 것은 괜찮습니다 — 표준은 배열 끝
바로 다음을 가리키는 포인터를 만드는 것을 명시적으로 허용합니다. 미묘한 부분은, `p`와 `b`가 주소를
공유하고도 같은 포인터가 아닐 수 있다는 것입니다. `p`는 자기가 `a`에서 왔다는 것을 기억하기 때문입니다.
...
둘 다 **경고 한 줄 없이** 걸려들 수 있고, 둘 다 `-O0`에서는 옳고 `-O2`에서는 틀린 코드를 만듭니다 —
디버그 빌드에서 돌린 모든 테스트를 통과하고 살아남으므로, 가능한 최악의 실패 방식입니다. 각각을 직접
돌려 볼 수 있을 만큼 작게 옮기면 이렇습니다.

**Strict aliasing.** 바이트 버퍼를 폭이 다른 두 포인터로 읽는 것 — 손으로 짠 모든 파서와 직렬화기의
모습입니다:
...
**Provenance.** 타입 퍼닝은 전혀 없습니다 — 두 포인터 모두 `int *`입니다 — 오직 *유래*만 다릅니다:

```c
#include <stdio.h>
#include <string.h>
int y = 2, x = 1;                        /* 서로 인접 배치될 가능성이 높음 */
int main(void) {
    int *p = &x + 1;                     /* x에서 유래; 주소는 x 바로 다음 */
    int *q = &y;                         /* y에서 유래 */
    if (memcmp(&p, &q, sizeof p) == 0) { /* 두 포인터가 비트까지 동일 */
        *p = 11;                         /* 그 주소에 씀 */
        printf("y=%d  *p=%d  *q=%d\n", y, *p, *q);
    }
}
...
```text
gcc -O1 :  y=11  *p=11  *q=11    # 쓰기가 y에 닿음
gcc -O2 :  y=2   *p=11  *q=2     # 같은 주소인데도 쓰기가 y에 닿지 않음
```

`-O2`에서 `p`를 통한 쓰기는 비트까지 `&y`인 주소에 떨어지고, `*p`는 `11`로 읽히는데, `y`는 여전히
`2`입니다. 컴파일러는 `p`가 `x`에서 왔음을 알기에 `y`를 건드릴 수 없다고 가정하고 `y`를 레지스터에
붙들어 둡니다 — 그래서 같은 주소가 동시에 두 값을 담습니다.
