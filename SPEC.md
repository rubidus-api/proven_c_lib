# proven C Library Specification (v26.05.19s)
...
1. C23 first: build with `-std=c23` when the compiler accepts it; the build driver falls back to `-std=c2x` on toolchains that still use the transitional spelling.
...
- `ring.h`: bounded ring buffer; not thread-safe by itself.
- `map.h`: open-addressing hash map with borrowed or owned U8 string keys.
...
- Maps use open addressing and tombstones. Integer keys and borrowed or owned U8 string keys are supported; borrowed keys must outlive the map, and debug validation or `PROVEN_HARDENED` can reject some internal-pointer misuse.
...
`-build-root <dir>` redirects generated output. Use `/home/user/work/build/proven_c_lib` on shared build servers.
...
- Linux x86_64 with GCC or Clang and C23 support. The build driver probes `-std=c23` first and falls back to `-std=c2x` when needed on older front ends.
