...
int main(int argc, char **argv)
{
    ...
    const Proven_Test_Case all_tests[] = {
        ...
        { "tests/test_regression_time_fmt_neg_year", "time formatting: u8 and u16 agree on zero-filled negative years", "Verify proven_time_u8_fmt and proven_time_u16_fmt render the same string for a zero-filled negative year - {year:0>4} of -44 is \"-044\" in both, with the sign counted toward the field width like printf %0Nd - and that positive years still agree.", "Inspect proven_sys_time_format_int_u16 in platform/proven_sys_time.c: the sign must count toward the pad width. The u16 path padded to full width THEN prepended the sign, one column wider than the fmt.h-based u8 path." },
