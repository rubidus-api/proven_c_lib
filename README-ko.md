...
- 버전: proven_c_lib-v26.09.03a · 표준: C23 · 라이선스: MIT
...
### "그 예제는 인위적이다" — 맞습니다, 그리고 그 이유가 흥미로운 지점입니다

실행 가능한 provenance 시연은 항상 이런 모습입니다. 인접한 변수 둘, 그리고 하나에서 다른 하나로
건너가는 포인터. 이건 상상력의 부족이 아니라 규칙의 작동 방식이 강요하는 것입니다. 컴파일러는 포인터가
어디서 왔는지 *볼 수 있을 때만* provenance를 이용하는데, 그 가시성이야말로 작은 예제는 가지고 있고
현실적인 예제는 `malloc`이나 함수 호출 뒤로 숨기는 바로 그것입니다. 컴파일러가 유래를 증명할 수 있을
만큼 범위를 줄이면, 인위적으로 *보이는* 것이 나옵니다. 주문하면 오컴파일될 만큼 작기 *때문에* 그렇게
보이는 것이죠.

그렇다면 현실적 관용구 — 정말로 산술로 포인터를 되짓는 것들 — 은 어떨까요? 하위 비트에 플래그를 넣고
마스크로 지우는 태그 포인터, 이웃 하나를 `prev ^ next`로 저장하는 XOR 연결 리스트, `data[-1]`로 닿는
`refcount` 헤더, `uintptr_t`로 왕복시킨 포인터. 이 전부를 `-O2`와 `-O3`로 컴파일해 봤고, **하나도
빠짐없이 올바른 답을 냈습니다.** 운이 아닙니다. WG14가 고른 모델 — PNVI-ae-udi, "정수를 통하지 않는
provenance, 노출된 주소" — 은 그 관용구들이 계속 동작하도록 일부러 설계된 것입니다. 그것들을 금지하면
막대한 양의 실제 핵심 코드가 깨지기 때문입니다. 포인터를 정수로 캐스트하면 그 주소가 *노출*되고, 정수에서
되짚은 포인터는 주소가 노출된 어떤 객체든 닿을 수 있으며, `data[-1]`은 같은 전체 할당 안에 머뭅니다.
위원회가 관행에 맞춰 규칙을 구부린 것입니다.

그래서 정직하고 살짝 불편한 요약은 이렇습니다. *시연할 수 있는* provenance 버그는 작고 인위적인
것이며, 현실적 관용구는 대개 깨지지 **않습니다** — 오늘, 이 컴파일러에서는. 위험한 것은 바로 그 "대개"
입니다. 규칙은 여전히 거기 있고, 컴파일러가 그것을 실행할 만큼의 가시성을 갖지 못했을 뿐입니다. 한
함수를 다른 함수에 인라인하고, 컴파일러를 올리고, LTO를 켜면, 볼 수 없던 유래가 볼 수 있는 유래가
됩니다 — 그리고 수년간 모든 테스트를 통과하던 프로그램이, 로직은 하나도 건드리지 않은 빌드 시스템
변경 하나에 오컴파일됩니다.

퍼즐이 아니라 이미 프로덕션 코드처럼 생긴 사례 하나:

```c
static int head[64];
static int body[192];

long checksum(const int *p, int n) {   /* p는 head[]에서 유래 */
    long s = 0;
    for (int i = 0; i < n; i++) s += p[i];
    return s;
}

/* "head와 body는 이어 붙어 있으니 한 번에 합치자": */
long total = checksum(head, 64 + 192);   /* head[]의 끝을 넘어감 */
```

버퍼를 도는 루프의 카운트가 하나 어긋난 것 — 세상에서 가장 평범한 버그입니다. `checksum`은 `head`에서
유래한 포인터와 그것을 넘어서는 길이를 받고, 의도는 뻔합니다. 두 배열을 더하려는 것이죠. 컴파일해서
무엇이 나오는지 보세요:

```text
gcc -O0 :  448   # 256개를 다 읽으며 head[]의 끝을 넘어 뒤따르는 메모리로 걸어감(미정의)
gcc -O2 :  64    # 컴파일러가 p는 head[64]에서 왔음을 알기에 읽기가 그 안에 머문다고 가정하고,
                 #   여러분의 256번 반복 중 192번을 조용히 버림 — 경고 없음
```

어느 답도 저자가 그렸던 "두 배열의 합"이 아니며, 둘은 최적화 레벨만으로 서로 어긋납니다. 여기에는
포인터를 캐스트하거나, 정수로 세탁하거나, 이상해 보이는 것이 아무것도 없습니다. 경계가 틀린 루프
하나일 뿐이고, *포인터가 어디서 왔는지*에 대한 컴파일러의 앎이 그것을 두 개의 다른 프로그램으로
바꿔 놓았습니다. 이것이 실제 provenance 버그가 배포되는 모습입니다. 주문하면 재현되는 크래시가 아니라,
올바르게 보이는 함수에 여러분의 툴체인이 정해 둔 유통기한이 붙어 있는 것.

그래서 이 라이브러리의 답은 "조심해서 provenance 깨끗한 코드를 쓰자"가 아닙니다. 이렇게 미묘한 규칙을
상대로 그걸 안정적으로 해낼 수 있는 사람은 없습니다. 답은 원시 포인터 산술을 작고 감사된 한 곳에
가두는 것입니다 — view는 포인터 *그리고* 길이이고, 컨테이너는 자기 범위를 압니다 — 그래서 "두 버퍼를
하나인 것처럼 처리"하는 일을 실수로는 쓸 수 없게 만드는 것입니다.
