...
static void require_text_missing(const char *text, const char *needle, const char *label) {
    PROVEN_TEST_ASSERT(strstr(text, needle) == NULL, label, "Inspect src/proven/float_format.c if the shared shortest common helper returns.");
}

int main(void) {
    PROVEN_TEST_SUITE(
        "test_float_shortest_shared",
        "Verify the shortest float-format backend keeps the shared parser-driven round-trip search helper and the thin per-width policy shims that feed it.",
        "Inspect src/proven/float_format.c if the shared helper disappears or if the per-width policy shims stop forwarding into it."
    );
    ...
    require_text_missing(src, "proven_float_format_find_shortest_precision", "shared shortest search helper should remain removed");
    free(src);

    PROVEN_TEST_PASS("Shared shortest-format helper checks passed.");
    return 0;
}
