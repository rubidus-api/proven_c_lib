...
The two examples above use an arena and a pool the way most code does. This one
covers the three things you reach for once the program is real:

1. **Allocation during start-up.** Some allocations have no sensible failure
   path: if the program cannot get its own configuration table, there is nothing
   left for it to do. `proven_arena_alloc_or_panic()` and
...
A handler that returns turns every later out-of-memory panic into a call that
