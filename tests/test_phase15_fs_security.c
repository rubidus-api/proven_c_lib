...
int main() {
    ...
    {
        // Create file
        proven_result_file_t r = proven_fs_open(test_file, PROVEN_FS_WRITE | PROVEN_FS_CREATE);
        ...
        PROVEN_TEST_ASSERT(PROVEN_IS_OK(proven_fs_chmod(test_file, ro_perms)), "Testing condition: PROVEN_IS_OK(proven_fs_chmod(test_file, ro_perms))", "Review logic surrounding PROVEN_IS_OK(proven_fs_chmod(test_file, ro_perms))");
        
        // Change back to Default
        PROVEN_TEST_ASSERT(PROVEN_IS_OK(proven_fs_chmod(test_file, PROVEN_FS_PERM_DEFAULT)), "Testing condition: PROVEN_IS_OK(proven_fs_chmod(test_file, PROVEN_FS_PERM_DEFAULT))", "Review logic surrounding PROVEN_IS_OK(proven_fs_chmod(test_file, PROVEN_FS_PERM_DEFAULT))");
    }

    // 2. Advisory Locking Test
    {
        proven_result_file_t r = proven_fs_open(test_file, PROVEN_FS_READ | PROVEN_FS_WRITE);
        if (!PROVEN_IS_OK(r.err)) {
            // Might fail if we can't open for write
            r = proven_fs_open(test_file, PROVEN_FS_READ);
        }
        PROVEN_TEST_ASSERT(PROVEN_IS_OK(r.err), "Testing condition: PROVEN_IS_OK(r.err)", "Review logic surrounding PROVEN_IS_OK(r.err)");
    ...
    // Cleanup
    (void)proven_fs_remove(test_file);

    PROVEN_TEST_INFO("All Phase 15 File Security & Locking Tests Passed Successfully!");
    return 0;
}
