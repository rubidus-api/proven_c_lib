...
int main(int argc, char **argv)
{
    ...
    const Proven_Test_Case all_tests[] = {
        ...
        { "tests/test_u8str_borrow", "U8 string borrow (fixed-capacity over caller memory)", "Verify proven_u8str_borrow/_reset: fixed-capacity ops and fmt work, growing ops refuse to reallocate caller memory, and destroy is a no-op.", "Inspect proven_u8str_borrow/_reset and the borrowed-flag guards in reserve/append_grow/replace_at_grow/destroy." },
        { "tests/test_mem_copy", "bounded memory copy", "Verify proven_mem_copy copies within capacity, rejects overflow without writing, treats a zero-size source as a no-op, and rejects null pointers.", "Inspect proven_mem_copy in src/proven/memory.c if a copy overflows, writes on rejection, or mishandles empty/null inputs." },
